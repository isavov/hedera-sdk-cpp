...
protected:
  ...
  [[nodiscard]] inline const Hbar& getAmount() const { return mAmount; }
  [[nodiscard]] inline const Hbar& getFeee() const { return mFee; }
...
private:
  ...
  const Hbar mAmount = Hedera::Hbar(505);
  const Hbar mFee = Hedera::Hbar(100000000);
};
...
TEST_F(SerializeAndDeserializeTest, SerializeAccountInfo)
{
...
TEST_F(SerializeAndDeserializeTest, DeserializeAccountInfo)
{
//   AccountBalanceQuery accountBalanceQuery;
  // accountBalanceQuery.setNodeAccountIds({ getTestAccountId() });
  // accountBalanceQuery.setAccountId(getTestAccountId());
